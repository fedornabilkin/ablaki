...
работают три недели. Владелец уточнил: в прежней production .env переменной
`COMPOSE_PROJECT_NAME` не было. Добавлять её не нужно. Если она была добавлена
во время настройки автоматизации, удалите эту строку целиком, сохранив прежнее
отсутствие переопределения. `apiablakinru` — наблюдаемое имя существующего проекта,
а не обязательная новая настройка. Тестовый проект — `ablaki`.
