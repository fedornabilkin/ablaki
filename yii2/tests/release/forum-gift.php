...
try {
    ...
    for ($i = 0; $i < 8; $i++) {
        $args = [PHP_BINARY, '-n', '-d', 'extension_dir=' . ini_get('extension_dir'), '-d', 'extension=pdo_sqlite', __FILE__, 'worker', $file, (string)$start];
        $pipes = [];
        $process = proc_open(implode(' ', array_map('escapeshellarg', $args)), [0 => ['pipe', 'r'], 1 => ['pipe', 'w'], 2 => ['pipe', 'w']], $pipes);
