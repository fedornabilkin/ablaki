...
function authWithoutCredentialWrites(Application $app, string $action, array $params = [])
{
    $before = $app->db->createCommand('SELECT * FROM user ORDER BY id')->queryAll();
    $protected = array_diff(array_keys($before[0]), ['last_login_at']);
    $app->db->pdo->exec('CREATE TRIGGER reject_login_writes BEFORE UPDATE OF '
        . implode(', ', $protected) . ' ON user BEGIN SELECT RAISE(ABORT, \'login must not write credentials\'); END');
    try {
        $result = $app->runAction('site/' . $action, $params);
        $after = $app->db->createCommand('SELECT * FROM user ORDER BY id')->queryAll();
        foreach ($before as &$row) unset($row['last_login_at']);
        unset($row);
        foreach ($after as &$row) unset($row['last_login_at']);
        unset($row);
        authResponseCheck($before === $after, 'login changes no user data except last_login_at');
        return $result;
    } finally {
...
try {
    ...
    foreach ([false, true] as $hasDescription) {
        ...
        foreach (['login-key', 'login'] as $action) {
            ...
            $response = authWithoutCredentialWrites($app, $action, $action === 'login-key' ? ['key' => $key] : []);
            ...
            authResponseCheck($response['token'] === $key, 'login returns the existing key without rotating it');
        }
        $app->user->setIdentity(null);
    ...
    foreach ([null, '', '   '] as $emptyKey) {
        ...
        $response = authWithoutCredentialWrites($app, 'login');
        authResponseCheck($app->user->isGuest && $app->response->statusCode === 401 && !isset($response['token'])
            && $db->createCommand('SELECT auth_key FROM user WHERE id=1')->queryScalar() === $emptyKey,
            'login rejects missing access key without creating one or returning an unusable token');
    }
